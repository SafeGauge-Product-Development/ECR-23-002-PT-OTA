...
{
      if (bleConnectRequestFlag2 == RESET)
      {
         // Error trying to disconnect before it has connected
         bleConnectRequestFlag1 = SET;
         blueState = BLE_IN_RESET;
         //shootCommsData(DISCONNECTION_ERROR);
      }
      else
      {
         ...
         //shootCommsData(BT_HAS_DISCONNECTED);
      }
}
...
void ble_evt_attclient_procedure_completed(const struct ble_msg_attclient_procedure_completed_evt_t *msg)
{
   switch(blueState)
   {
      ...
      //--------------------------------------
      case BLE_WAITING_FOR_SERVER_ATTRIBUTES_RESPONSE: // 23 // this is the second condition after a write
      {
         blueState = BLE_IDLE;
         blueTimer = 0;          
...
{ 
   ...
   for (n = 0; n < 3; n++)
   {
      if (serialNumbers[2 - n] != msg->data.data[5 + n])
         match = NO;
   }       
   //---
   for (n = 0; n < 13; n++)
